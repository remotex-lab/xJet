...
export class TestModel {
     ...
     * or `undefined` if no location information is available.
     *
     * @remarks
     ...
     * or `undefined` to clear the current location.
    ...
    async execute(context: ContextType<ContextInterface>, executeHooks: DescribeModel['executeHooks'], isOnlyMode: boolean = false): Promise<void> {
        ...
        await this.dispatchTestEvent(ActionType.START);
        ...
        try {
            ...
            await this.dispatchTestEvent(ActionType.FAILURE, [ error ]);
        }
    }
    ...
    private async shouldSkipExecution(context: ContextType<ContextInterface>, isOnlyMode: boolean): Promise<boolean> {
        if (this.testFlags.todo)
            return await this.dispatchTestEvent(ActionType.TODO) ?? true;

        if (this.testFlags.skip || isOnlyMode)
            return await this.dispatchTestEvent(ActionType.SKIP) ?? true;

        if (context.beforeAllErrors && context.beforeAllErrors.length > 0) {
            return await this.dispatchTestEvent(
                ActionType.FAILURE, <Array<unknown>> context.beforeAllErrors[context.beforeAllErrors.length - 1]
            ) ?? true;
        }
    ...
    private async handlePostExecution(): Promise<void> {
        ...
        await this.dispatchTestEvent(ActionType.SUCCESS);
    }
    ...
    private async dispatchTestEvent(action: ActionType, errors?: Array<unknown>): Promise<void> {
        emitService.testDispatcher({
            action: action,

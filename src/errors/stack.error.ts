...
function formatErrorLine(name: string, file: string, line: number, column: number, frame: NodeJS.CallSite): string {
    ...
    const position = (line >= 0 && column >= 0) ? setColor(Colors.gray, `[${ line }:${ column }]`, true) : '';

    return `at ${ formattedName } ${ setColor(Colors.darkGray, file, true) } ${ position }`
        .replace(/\s{2,}/g, ' ').trim();
}
...
function highlightPositionCode(position: PositionWithCodeInterface, error: ErrorType): string {
    ...
    return formatErrorCode({ ...position, code: highlightCode(position.code) }, {
...
function formatStackEntry(frame: NodeJS.CallSite, state: StackTraceStateInterface): string {
    ...
    const isXJetService = source === xJetLazy.service.file;

    if (isXJetService)
        position = xJetLazy.service.getPositionWithCode(line, column);
    else if (state.error.sourceMap)
        position = state.error.sourceMap.getPositionWithCode(line, column);

    if (position) {
        const dirPath = isXJetService ? distPath : cliPath;
        ...
        if (position.sourceRoot)
            source = modifyFilePathWithSourceRoot(
                ...
                position.sourceRoot,
                position.line,
                isXJetService ? rootPath : cliPath
            );

...
import type { ConstructorKeysType, KeysExtendingConstructorType } from '@global/mock/interfaces/spy-mock.interface';
import type { FunctionType, ConstructorType, FunctionLikeType, ConstructorLikeType } from '@interfaces/function.interface';

/**
 * Imports
 */

import { MockState } from '@global/states/mock.state';
...
export function spyOn<Target, Key extends KeysExtendingConstructorType<Target>>(target: Target, key: Key): Target[Key] extends FunctionType
...
export function spyOn<Target, Key extends ConstructorKeysType<Target>>(target: Target, key: Key): Target[Key] extends ConstructorType
...
export function spyOn<Target, Key extends keyof Target>(target: Target, key: Key): Target[Key] extends FunctionType
...
export function spyOn<T extends object, K extends keyof T>(target: T, key: K): MockState {
    const location = getInvocationLocation();
    if (target == null || (typeof target !== 'object' && typeof target !== 'function'))
        throw new ExecutionError(`Invalid spy target: expected object / function, received ${ typeof target }`, location);

    if (key === null)
        throw new ExecutionError('Property access failed: property name must be provided', location);

    const item = <FunctionLikeType | ConstructorLikeType> target[key];
    if (!item) {
        throw new ExecutionError(`Cannot access property: '${
            String(key)
        }' is undefined or not available on target object`, location);
    }

...
export class DescribeModel {
    ...
    async execute(context: ContextType<ContextInterface>): Promise<void> {
        ...
        await this.dispatchDescribeEvent(StatusType.START_DESCRIBE);
        ...
        await this.dispatchDescribeEvent(StatusType.SUCCESS_DESCRIBE);
        for(const error of context.afterAllErrors) {
            await this.dispatchDescribeEvent(StatusType.FAILURE, error);
        }
    ...
    /**
     * Dispatches a descriptive event with the provided status and optional error.
     *
     * @param status - The status of the event to be dispatched.
     * @param error - Optional, The error information related to the event, if any.
     * @return A promise that resolves when the event has been dispatched.
     *
     * @remarks
     * This method constructs an event object using the provided parameters and internal properties such as
     * `parents`, `getDuration`, and `name`, before delegating the dispatching to `dispatcherComponent`.
     *
     * @since 1.0.0
     */

    private async dispatchDescribeEvent(status: StatusType, error?: unknown): Promise<void> {
        dispatcherComponent({
            error,
            status,

...
import type { FnMockInterface } from '@global/mock/interfaces/fn-mock.interface';
 ...
 * const mock = fnMock((x: number) => x * 2);
 * console.log(mock(5)); // 10
 *
 * // Creating a mock with a restore function
 * const mockWithRestore = fnMock(undefined, () => { console.log('Restored!'); });
...
export function fnMock<ReturnType, Args extends Array<unknown>, Context>(
...
export function mock<Method, Args extends Array<unknown>, Context>(
    ...
    if (!parentObject) {
        throw new ExecutionError('Cannot mock function: the provided function is not provided', getInvocationLocation());
    }

    const originalMethod = method;
    if (method.prototype && !Object.getOwnPropertyDescriptor(method, 'prototype')?.writable) {
        parentObject[method.name] = new MockState<Method, Args, Context>(
    ...
    if (typeof method === 'function') {
        parentObject[method.name] = new MockState(<FunctionLikeType> method, () => {
            parentObject[method.name] = originalMethod;
        });
    ...
    throw new ExecutionError('Unsupported target: method must be a valid function or constructor', getInvocationLocation());
}

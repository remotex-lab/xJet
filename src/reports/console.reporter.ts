...
export default class ConsoleReporter extends AbstractReporter {
    ...
    finish(): void {
        ...
        if(this.state.passedSuites > 0) {
            suites += ` ${ setColor(Colors.PastelGreen, `${ this.state.passedSuites } passed`) }`;
        }

        if(this.state.failedSuites > 0) {
            suites += ` ${ setColor(Colors.BrightPink, `${ this.state.failedSuites } failed`) }`;
        }

        if(this.state.skippedSuites > 0) {
            suites += ` ${ setColor(Colors.LightGoldenrodYellow, `${ this.state.skippedSuites } skipped`) }`;
        }

        if(this.state.totalSuites > 0) {
            suites += ` ${ this.state.totalSuites } total`;
        }

        if(this.state.passed > 0) {
            tests += ` ${ setColor(Colors.PastelGreen, `${ this.state.passed } passed`) }`;
        }

        if(this.state.failed > 0) {
            tests += ` ${ setColor(Colors.BrightPink, `${ this.state.failed } failed`) }`;
        }

        if(this.state.skipped > 0) {
            tests += ` ${ setColor(Colors.LightGoldenrodYellow, `${ this.state.skipped } skipped`) }`;
        }

        if(this.state.todo > 0) {
            tests += ` ${ setColor(Colors.PastelPurple, `${ this.state.todo } skipped`) }`;
        }

        if(this.state.total > 0) {
            tests += ` ${ this.state.total } total`;
        }

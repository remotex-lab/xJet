...
/**
 * Custom implementation of Error.prepareStackTrace that captures CallSite objects.
 *
 * @param error - The Error object being processed
 * @param stackTraces - Array of CallSite objects representing the stack trace
 * @returns A string representation of the stack trace
 *
 * @remarks
 * This function extends the standard Error.prepareStackTrace functionality by
 * attaching the raw CallSite objects to the error object for later inspection.
 * It preserves the original behavior by calling the original prepareStackTrace
...
Error.prepareStackTrace = (error: Error, stackTraces: Array<NodeJS.CallSite>): string => {
    const errorObject = error as BaseError;
    // Attach the stack entries (CallSite objects) to the error object
    errorObject.callStacks = stackTraces;

    // Call the original prepareStackTrace to return the standard string representation of the stack
    return originalPrepareStackTrace ? originalPrepareStackTrace(errorObject, stackTraces) : '';
};
...
export abstract class BaseError extends Error {
    ...
    /**
     * Represents an array of stack trace call site objects.
     *
     * @see NodeJS.CallSite
     * @since 1.0.0
     */

    protected stacks: Array<NodeJS.CallSite> = [];
    ...
    /**
     * Gets the call stack entries.
     *
     * @returns Array of CallSite objects representing the stack trace
     *
     * @see NodeJS.CallSite
     * @since 1.0.0
     */
    ...
    get callStacks(): Array<NodeJS.CallSite> {
        return this.stacks;
    }

    /**
     * Sets the call stacks for the current instance.
     *
     * @param stacks - An array of `NodeJS.CallSite` objects representing the call stack.
     *
     * @see NodeJS.CallSite
     * @since 1.0.0
     */

    set callStacks(stacks: Array<NodeJS.CallSite>) {
        this.stacks = stacks;
    }

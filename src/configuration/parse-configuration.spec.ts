...
describe('patchEventEmitterErrors', () => {
    ...
    test('should throw original error when error is a framework error', () => {
        // Given
        const emitter = new EventEmitter();
        const testError = new Error('Framework error');
        ...
        // Mock VMRuntimeError to report it IS a framework error
        (VMRuntimeError as unknown as jest.Mock).mockImplementation(() => ({
            isNotFrameworkError: jest.fn().mockReturnValue(false),
            toString: () => 'VMRuntimeError'
        }));

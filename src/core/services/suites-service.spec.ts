...
jest.mock('@adapters/local.adapter', () => {
    return {
        LocalAdapter: jest.fn().mockImplementation(() => ({
            initAdapter: jest.fn(),
...
jest.mock('@adapters/external.adapter', () => {
    return {
        ExternalAdapter: jest.fn().mockImplementation(() => ({
            initAdapter: jest.fn(),
...
describe('SuitesService', () => {
    ...
    describe('constructor', () => {
        test('should initialize with local adapter when testRunners is not configured', () => {
            mockConfig.testRunners = undefined;
            new SuitesService(mockConfig);

            expect(LocalAdapter).toHaveBeenCalledWith(mockConfig);
            expect(ExternalAdapter).not.toHaveBeenCalled();
            expect(SpecsProvider).toHaveBeenCalledWith(mockConfig);
        });

        test('should initialize with external adapter when testRunners is configured', () => {
            mockConfig.testRunners = <any> [ 'runner1', 'runner2' ];
            new SuitesService(mockConfig);

            expect(LocalAdapter).not.toHaveBeenCalled();
            expect(ExternalAdapter).toHaveBeenCalledWith(mockConfig);
        });
    });
    ...
    describe('executeSuite', () => {
        test('should transpile files and execute suites without watching', async() => {
            ...
            const suitesService = new SuitesService(mockConfig);
    ...
    describe('watchForChanges', () => {
        ...
        beforeEach(() => {
            mockConfig.watch = true;
            suitesService = new SuitesService(mockConfig);
        });

...
/**
 * `SourceServiceSingleton` is a singleton class that provides a single instance of the `SourceService` class.
 *
 * It ensures that only one instance of `SourceService` is created during the application lifecycle.
 * The instance is lazily initialized when `getInstance` is called for the first time. It reads and parses
 * a source map from `index.js.map` and passes it to the `SourceService` constructor.
 *
 * ### Example Usage:
 * ```typescript
 * const sourceService = SourceServiceSingleton.getInstance();
 * ```
 *
 * todo refactor reading of the source-map
 */

export class SourceServiceSingleton {
    private static instance: SourceService;

    static getInstance(): SourceService {
        if (!SourceServiceSingleton.instance) {
            const sourceMapData = readFileSync(join(__dirname, 'index.js.map'));
            SourceServiceSingleton.instance = new SourceService(JSON.parse(sourceMapData.toString()));
        }

        return SourceServiceSingleton.instance;
    }
}
...
function resolveSourcePosition(stackEntry: StackEntryInterface, sourceService: SourceService): PositionSourceInterface | null {
    if (stackEntry.file.match(/evalmachine|anonymous|node:vm/))
        return sourceService.getSourcePosition(stackEntry.line, stackEntry.column);

    return SourceServiceSingleton.getInstance().getSourcePosition(stackEntry.line, stackEntry.column);
}
...
export abstract class BaseError extends Error {
    ...
    protected constructor(message: string, protected sourceMap: SourceService = SourceServiceSingleton.getInstance()) {
        super(message);
    ...
    protected getFormattedStackEntries(stackEntries: StackEntryInterface[], sourceMap: SourceService): string[] {
        return stackEntries.map((item) => {
            ...
            // Todo fix unresolvable error that point to different location
